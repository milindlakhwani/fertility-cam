...
class Camera
  ...
  public Camera(
    ...
    cameraCaptureCallback = new CameraCaptureCallback(Camera.this, captureTimeouts, captureProps) {
        @Override
    ...
    CameraCaptureSession.StateCallback callback =
        new CameraCaptureSession.StateCallback() {
          ...
          public void onConfigured(@NonNull CameraCaptureSession session) {
            ...
            Log.i(TAG, "Updating builder settings (ABCD)");
  ...
  */
  ...
  public void setExposureOffset(@NonNull final Result result, double offset) {
     ...
     *
     * @param result The MethodChannel.Result used to send back the exposure value to Flutter.
     */  
  public void getExposureOffset(@NonNull final Result result) {
    refreshPreviewCaptureSession(
        () -> result.success(latestExposureOffset),
        (code, message) ->
            result.error("getExposureOffsetFailed", "Exposure offset not available", null));
  }
